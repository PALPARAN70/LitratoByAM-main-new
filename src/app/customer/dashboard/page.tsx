...
export default function DashboardPage() {
  ...
  const getAuthHeader = useCallback((): HeadersInit => {
    const token =
      ...
      null
    // Always return a HeadersInit-compatible object (Record<string, string>)
    return token
      ...
      const mapped: Row[] = bookingsRaw
        ...
        .map((b) => {
          const timeStart = String(
            (b as Record<string, unknown>).event_time ??
              (b as Record<string, unknown>).eventtime ??
              ''
          ).slice(0, 5)
          const timeEnd = String(
            (b as Record<string, unknown>).event_end_time ?? ''
          ).slice(0, 5)
          const dateISO = toISODate(
            String(
              (b as Record<string, unknown>).event_date ??
                (b as Record<string, unknown>).eventdate ??
                ''
            )
          )
          // Derive grid names from either grid_names array or legacy grid string
          const gridNames: string[] = Array.isArray(
            (b as Record<string, unknown>).grid_names
          )
            ? ((b as Record<string, unknown>).grid_names as unknown[])
                .map((n) => String(n ?? '').trim())
                .filter((s) => !!s)
            : String(((b as Record<string, unknown>).grid ?? '') as string)
                .split(',')
                .map((s) => s.trim())
                .filter((s) => !!s)
          ...
          return {
            ...
            package: String(
              (b as Record<string, unknown>).package_name ?? ''
            ).trim(),
            grid: gridString,
            place: String(
              (b as Record<string, unknown>).event_address ??
            ...
            requestid:
              (b as Record<string, unknown>).requestid != null
                ...
                : undefined,
            contact_info:
              ((b as Record<string, unknown>).contact_info as string | null) ??
              null,
            contact_person:
              ((b as Record<string, unknown>).contact_person as
                | string
                | null) ?? null,
            contact_person_number:
              ((b as Record<string, unknown>).contact_person_number as
                | string
                | null) ?? null,
            strongest_signal:
              ((b as Record<string, unknown>).strongest_signal as
                | string
                | null) ?? null,
            extension_duration:
              typeof (b as Record<string, unknown>).extension_duration ===
              'number'
                ? ((b as Record<string, unknown>).extension_duration as number)
                : (b as Record<string, unknown>).extension_duration != null
                ? Number(
                    (b as Record<string, unknown>)
                      .extension_duration as unknown as number
                  )
                : null,
  ...
  return (
    <MotionDiv>
                    ...
                    {paginated.length === 0 ? (
                      ...
                      paginated.map((data, i) => (
                        ...
                        >
                          ...
                          <td className="px-3 sm:px-4 py-2 whitespace-nowrap">
                            {(() => {
                              const byField = (data.contact_person || '').trim()
                              const byProfile = [
                                profile?.firstname,
                                profile?.lastname,
                              ]
      ...
      {/* ADD: Edit (formerly Reschedule) confirmation dialog */}
      <Dialog
        open={reschedOpen}
        onOpenChange={(o) => {
          if (!o) {
